...
function _treeGetFilterBySearch(treeData: Array<any>, scFunc: TreeSearchFunc, idSet: TreeIdSet, opt: TreeFilterOption): Array<any> {
    ...
    if (parentMatch) {
        ...
        for (const treeNode of cloneTreeData) {
            ...
            if (Array.isArray(children) && children.length) {
                if (childrenMatch) {
                    setTreePropsValue(treeNode, "children", _treeGetFilterBySearch(children, scFunc, idSet, opt), opt);
                }
            } else {
...
export default function treeGetFilterBySearch(treeData: Array<any>, scFunc: TreeSearchFunc, opt: TreeFilterOption): Array<any> {
    ...
    return _treeGetFilterBySearch(treeData, scFunc, idSet, opt);
}
