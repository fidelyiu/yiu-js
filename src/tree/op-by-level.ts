...
function _opByLevel(treeData: Array<any>, opFunc: TreeOperationFunc, currentLevel: number, targetLevel: number, opt?: TreeBaseOpt): Array<any> {
    treeData.forEach((item) => {
        const children = getTreePropsValue(item, "children", opt);
        if (Array.isArray(children) && children.length > 0) {
            setTreePropsValue(item, "children", _opByLevel(children, opFunc, currentLevel + 1, targetLevel, opt), opt);
        }
...
export default function treeOpByLevel(treeData: Array<any>, opFunc: TreeOperationFunc, level = 1, opt?: TreeBaseOpt): Array<any> {
    ...
    return _opByLevel(treeData, opFunc, 1, level, opt);
}
